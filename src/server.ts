...
function detectFeaturesFromDeps(deps: Record<string, string> | null | undefined) {
  const f: string[] = [];
  if (!deps) return f;
  const has = (k: string) => Object.prototype.hasOwnProperty.call(deps, k);
  if (has("express")) f.push("基于 Express 的 Web 服务");
  if (has("mongoose")) f.push("使用 Mongoose 访问 MongoDB");
  if (has("jsonwebtoken")) f.push("JWT 鉴权");
  if (has("bcrypt")) f.push("密码哈希（bcrypt）");
  if (has("morgan")) f.push("HTTP 请求日志（morgan）");
  if (has("cookie-parser")) f.push("Cookie 解析");
  if (has("body-parser")) f.push("请求体解析（body-parser）");
  if (has("jade") || has("pug")) f.push("模板引擎 (Jade/Pug)");
  return f;
}

function buildBadges(opts: { license?: string | null; node?: string | null; ts?: boolean }) {
  const parts: string[] = [];
  if (opts.license) {
    parts.push(
  ...
  if (opts.node) {
    parts.push(
      `![Node](https://img.shields.io/badge/node-${encodeURIComponent(String(opts.node))}-brightgreen)`
    );
  }
  if (opts.ts) {
...
async function buildTree(dir: string, depth: number) {
  const patterns = [
    "**/*",
    "!node_modules/**",
    "!dist/**",
...
async function grepEnvKeys(dir: string) {
  const patterns = [
    "**/*.js",
    "**/*.ts",
    "!node_modules/**",
...
async function grepHttpRoutes(dir: string) {
  const patterns = ["**/*.js", "!node_modules/**", "!dist/**", "!.git/**"];
  const files = await fg(patterns, { cwd: dir });
  const routes: string[] = [];
  const method =
    /\b(app|router)\.(get|post|put|delete|patch|options|head)\s*\(\s*["'`]([^"'`]+)["'`]/g;
...
function renderScripts(scripts: Record<string, string> | null | undefined) {
  if (!scripts || Object.keys(scripts).length === 0) return "(无脚本)";
  const lines = Object.entries(scripts).map(([k, v]) => `npm run ${k}  # ${v}`);
  return lines.join("\n");
}

function renderTechStack(deps: Record<string, string> | null | undefined) {
  if (!deps) return "- (未检测到依赖)";
 ...
 * README 生成：基础版
 * ------------------------------------- */
function buildReadmeBasic(input: z.infer<typeof GenerateReadmeInput>): string {
...
async function main() {
  ...
  server.registerTool(
    ...
    async (args: unknown) => {
      ...
      const pkg = await readJSON<any>(path.join(dir, "package.json"));
      ...
      const language: Lang = parsed.language ?? "zh";

      // 徽章
      let licenseText: string | null = null;
      const hasLicenseFile =
        (await fileExists(path.join(dir, "LICENSE"))) ||
